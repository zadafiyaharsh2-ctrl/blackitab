      ...
      <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}
        ...
        <div className="text-center mb-8">
          <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ type: "spring", stiffness: 200, delay: 0.2 }}
            className="w-16 h-16 mx-auto mb-4 rounded-2xl bg-gradient-to-br from-emerald-500/20 to-blue-500/20 border border-white/10 flex items-center justify-center shadow-inner">
            {step === 1 ? <User className="w-8 h-8 text-emerald-400" /> : <GraduationCap className="w-8 h-8 text-blue-400" />}
          </motion.div>
            ...
            <motion.div key="step1" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }}>
              <div className="space-y-5">
                ...
                <div className="space-y-1">
                  ...
                  <div className="relative group">
                    ...
                    <input type="text" name="name" value={formData.name} onChange={handleChange} required placeholder="John Doe"
                      className="w-full pl-12 pr-4 py-3 bg-white/5 border border-white/10 rounded-xl focus:ring-2 focus:ring-emerald-500/50 text-white placeholder-gray-600 outline-none transition-all" />
                  </div>
                </div>
                ...
                <div className="space-y-1">
                  ...
                  <div className="relative group">
                    <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                      <Lock className={`w-5 h-5 transition-colors ${passwordStrength ? 'text-emerald-400' : 'text-gray-500'}`} />
                    </div>
                    <input type={showPassword ? 'text' : 'password'} name="password" value={formData.password} onChange={handleChange} required placeholder="Min 6 characters"
                      className={`w-full pl-12 pr-12 py-3 bg-white/5 border rounded-xl outline-none transition-all ${formData.password && !passwordStrength ? 'border-red-500/50' : 'border-white/10 focus:ring-2 focus:ring-emerald-500/50'}`} />
                ...
                <div className="space-y-1">
                  ...
                  <div className="relative group">
                    <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                      <Lock className={`w-5 h-5 transition-colors ${passwordsMatch ? 'text-emerald-400' : 'text-gray-500'}`} />
                    </div>
                    <input type={showConfirmPassword ? 'text' : 'password'} name="confirmPassword" value={formData.confirmPassword} onChange={handleChange} required placeholder="Re-enter password"
                      className={`w-full pl-12 pr-12 py-3 bg-white/5 border rounded-xl outline-none transition-all ${formData.confirmPassword && !passwordsMatch ? 'border-red-500/50' : 'border-white/10 focus:ring-2 focus:ring-emerald-500/50'}`} />
            ...
            <motion.div key="step2" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
              <form ref={formRef} onSubmit={handleSubmit} className="space-y-5">
                ...
                <div className="flex items-center gap-3 p-4 rounded-xl bg-blue-500/10 border border-blue-500/20">
                  ...
                  <div>
                    <p className="text-white font-bold text-sm">Joining as Student</p>
                    <p className="text-gray-400 text-[10px]">Teachers, HODs & admins are promoted internally by their institute</p>
                  </div>
                </div>
                  ...
                  <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} className="grid grid-cols-2 gap-3">
                    <div className="space-y-1">
                      ...
                      <input type="text" name="batchYear" value={formData.batchYear} onChange={handleChange}
                        ...
                        className="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-xl focus:ring-2 focus:ring-purple-500/50 text-white placeholder-gray-600 outline-none" />
                    </div>
                    <div className="space-y-1">
