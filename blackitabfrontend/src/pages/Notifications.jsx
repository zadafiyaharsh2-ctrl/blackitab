        ...
        <div className="min-h-screen bg-transparent backdrop-blur-sm p-4 md:p-8 relative overflow-hidden">
            ...
            <div className="max-w-2xl mx-auto relative z-10">
                <div className="mb-8 flex items-center gap-4">
                    <button onClick={() => window.history.back()} className="p-3 mr-2 rounded-full bg-white/5 hover:bg-white/10 text-white transition-colors border border-white/10">
                        <FaArrowLeft />
                    </button>
                    <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-blue-500/20 to-purple-500/20 flex items-center justify-center border border-white/10 shadow-lg backdrop-blur-md">
                        <FaBell className="text-blue-400 text-xl" />
                    </div>
                    <div>
                        <h1 className="text-3xl font-bold text-white tracking-tight">Notifications</h1>
                        <p className="text-gray-400 text-sm">Stay updated with your network</p>
                    </div>
                </div>
                ...
                {loading ? (
                    ...
                    <div className="text-center py-20 bg-white/5 rounded-3xl border border-white/5 backdrop-blur-md">
                        <div className="w-20 h-20 bg-white/5 rounded-full flex items-center justify-center mx-auto mb-6">
                            <FaBell className="text-4xl text-gray-600" />
                        </div>
                        <h3 className="text-xl font-semibold text-white mb-2">No notifications yet</h3>
                        <p className="text-gray-400">When you get likes, comments or follows, they'll show up here.</p>
                    </div>
                ) : (
                        ...
                        <AnimatePresence>
                            {notifications.map((note) => (
                                <motion.div
                                    ...
                                    key={note._id}
                                    className="bg-white/5 hover:bg-white/10 border border-white/5 hover:border-white/10 rounded-2xl p-4 transition-all backdrop-blur-md shadow-lg"
                                >
                                                    ...
                                                    {!note.isAccepted ? (
                                                        <>
                                                            <button 
                                                                ...
                                                                className="px-4 py-1.5 bg-blue-600 hover:bg-blue-500 text-white text-xs font-semibold rounded-lg transition-colors flex items-center gap-2 shadow-lg shadow-blue-900/20"
                                                            >
                                                                <FaCheck size={10} /> Confirm
                                                            </button>
                                                            <button 
                                                                onClick={() => handleReject(note.sender._id, note._id)}
                                                                className="px-4 py-1.5 bg-white/5 hover:bg-white/10 border border-white/10 text-gray-300 text-xs font-semibold rounded-lg transition-colors flex items-center gap-2"
                                                            >
                                                        ...
                                                        !note.isFollowing ? (
                                                            <button 
                                                                ...
                                                                className="px-4 py-1.5 bg-gradient-to-r from-purple-600 to-blue-600 hover:from-purple-500 hover:to-blue-500 text-white text-xs font-semibold rounded-lg transition-all shadow-lg flex items-center gap-2"
                                                            >
