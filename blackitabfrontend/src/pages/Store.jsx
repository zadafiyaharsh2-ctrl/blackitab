    ...
    <div className="min-h-screen p-6">
          ...
          <div className="relative z-10">
            ...
            <div className="flex flex-wrap gap-4">
              {benefits.map((benefit, index) => {
                ...
                return (
                  <div key={index} className={`flex items-center gap-3 px-5 py-3 rounded-xl border ${isDark ? 'bg-gray-800/60 border-gray-700' : 'bg-white/80 border-gray-200'} backdrop-blur-sm`}>
                    <Icon className={`text-xl ${isDark ? 'text-pink-400' : 'text-pink-600'}`} />
                    <div>
                      <h3 className={`font-bold text-sm ${isDark ? 'text-white' : 'text-gray-900'}`}>{benefit.title}</h3>
      ...
      <div className="max-w-7xl mx-auto mb-12">
        <div className={`${isDark ? 'bg-gradient-to-r from-purple-900/30 to-pink-900/30 border-purple-700/50' : 'bg-gradient-to-r from-purple-50 to-pink-50 border-purple-200'} rounded-2xl p-8 border backdrop-blur-md`}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
            ...
            <div className={`${isDark ? 'bg-gray-800/60' : 'bg-white/90'} rounded-xl p-6 border ${isDark ? 'border-gray-700' : 'border-gray-200'} shadow-lg`}>
              ...
              <div className="flex gap-4 mt-4">
                <div className={`flex-1 p-3 rounded-lg ${isDark ? 'bg-gray-700' : 'bg-gray-100'} text-center`}>
                  ...
                  <p className={`text-xs font-bold ${isDark ? 'text-white' : 'text-gray-900'}`}>Pro Laptop</p>
                  <p className="text-xs text-green-500">98% Match</p>
                </div>
                <div className={`flex-1 p-3 rounded-lg ${isDark ? 'bg-gray-700' : 'bg-gray-100'} text-center`}>
                  <FaHeadphones className="text-3xl mx-auto mb-2 text-pink-400" />
                  <p className={`text-xs font-bold ${isDark ? 'text-white' : 'text-gray-900'}`}>Focus Buds</p>
                  <p className="text-xs text-green-500">95% Match</p>
                </div>
                <div className={`flex-1 p-3 rounded-lg ${isDark ? 'bg-gray-700' : 'bg-gray-100'} text-center`}>
                  <FaClock className="text-3xl mx-auto mb-2 text-orange-400" />
                  <p className={`text-xs font-bold ${isDark ? 'text-white' : 'text-gray-900'}`}>Smart Watch</p>
      ...
      <div className="max-w-4xl mx-auto text-center">
        <div className={`${isDark ? 'bg-gray-800/50 border-gray-700' : 'bg-white border-gray-200'} rounded-2xl p-8 md:p-12 border backdrop-blur-md shadow-xl`}>
          ...
          <button className="bg-gradient-to-r from-pink-600 to-purple-600 hover:from-pink-700 hover:to-purple-700 text-white font-bold py-4 px-8 rounded-xl shadow-lg hover:shadow-xl transition-all duration-300 transform hover:scale-105 text-lg">
            Notify Me When Live
          </button>
        </div>
      </div>
