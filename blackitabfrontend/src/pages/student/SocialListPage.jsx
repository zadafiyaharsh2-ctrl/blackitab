...
import { FaArrowLeft, FaEnvelope, FaSearch, FaUserMinus, FaTimes } from 'react-icons/fa';
...
const SocialListPage = () => {
  ...
  useEffect(() => {
    const fetchOwner = async () => {
      ...
      try {
        ...
        const res = await axios.get(`${API_URL}/api/social/user/${userId}`, {
  ...
  useEffect(() => {
    const fetchFollowers = async () => {
      ...
      try {
        const token = localStorage.getItem('token');
        const res = await axios.get(`${API_URL}/api/social/followers/${userId}`, {
          headers: { Authorization: `Bearer ${token}` }
        });
  ...
  useEffect(() => {
    const fetchFollowing = async () => {
      ...
      try {
        const token = localStorage.getItem('token');
        const res = await axios.get(`${API_URL}/api/social/following/${userId}`, {
          headers: { Authorization: `Bearer ${token}` }
        });
  ...
  useEffect(() => {
    window.history.replaceState(null, '', `/network/${userId}/${activeTab}`);
  }, [activeTab, userId]);
  ...
  const handleFollow = async (targetId) => {
    try {
      ...
      const updater = prev => prev.map(u =>
        u._id === targetId ? { ...u, isFollowing: isAccepted, isRequested: !isAccepted } : u
      );
  ...
  const executeUnfollow = async (targetId) => {
    try {
      ...
      const updater = prev => prev.map(u =>
        u._id === targetId ? { ...u, isFollowing: false, isRequested: false } : u
      );
      setFollowers(updater);
      setFollowing(updater);
    } catch (err) {
  ...
  const handleUnfollow = (targetId) => {
    setConfirmState({
      isOpen: true, action: executeUnfollow, id: targetId,
      title: 'Unfollow User', message: 'Are you sure you want to unfollow this user?'
    });
  };
  ...
  const executeRemoveFollower = async (targetId) => {
    try {
      const token = localStorage.getItem('token');
      await axios.post(`${API_URL}/api/social/remove-follower/${targetId}`, {}, {
        headers: { Authorization: `Bearer ${token}` }
      });
      ...
      console.error(err);
      toast.error(err.response?.data?.message || 'Error removing follower');
    }
  };

  const handleRemoveFollower = (targetId) => {
    setConfirmState({
      isOpen: true, action: executeRemoveFollower, id: targetId,
      title: 'Remove Follower', message: 'This will remove them from your followers. They can still follow you again later.'
    });
  };
  ...
  const renderUserCard = (u) => {
    const isSelf = (currentUserId) === (u._id || u.id);
        ...
        <div className="flex items-center gap-3 sm:gap-4 min-w-0 flex-1">
          ...
          <div className="min-w-0">
            ...
            <div className="text-xs sm:text-sm text-gray-500 dark:text-gray-400 font-medium">
              ...
              {u.bio && <span className="hidden sm:inline"> · {u.bio.substring(0, 40)}{u.bio.length > 40 ? '…' : ''}</span>}
            </div>
          </div>
        </div>
          ...
          <div onClick={(e) => e.stopPropagation()} className="flex items-center gap-2 sm:gap-3 flex-shrink-0 ml-2">
            ...
            {u.isFollowing ? (
              <button
                ...
                className="px-4 sm:px-6 py-2 sm:py-2.5 rounded-full border border-gray-300 dark:border-gray-600 hover:border-red-400 dark:hover:border-red-500/40 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-500/10 text-xs sm:text-sm font-bold transition-all text-gray-600 dark:text-gray-300"
              >
                Following
              </button>
            ) : u.isRequested ? (
              <button
                disabled
                className="px-4 sm:px-6 py-2 sm:py-2.5 rounded-full border border-gray-300 dark:border-gray-600 text-xs sm:text-sm font-bold text-gray-400 dark:text-gray-500 cursor-not-allowed"
              >
                Requested
              </button>
            ) : (
              <button
                ...
                className="px-4 sm:px-6 py-2 sm:py-2.5 rounded-full bg-blue-600 hover:bg-blue-500 text-white text-xs sm:text-sm font-bold transition-all shadow-lg shadow-blue-500/20 hover:shadow-blue-500/30 hover:scale-[1.03]"
              >
                Follow
              </button>
            )}
            ...
            {isOwnProfile && activeTab === 'followers' && (
              <button
                onClick={(e) => { e.stopPropagation(); handleRemoveFollower(u._id); }}
                className="p-2.5 rounded-full bg-gray-100 dark:bg-white/5 hover:bg-red-50 dark:hover:bg-red-500/10 text-gray-400 hover:text-red-500 transition-colors border border-gray-200 dark:border-white/10 hover:border-red-300 dark:hover:border-red-500/30"
      ...
      <div className="max-w-3xl mx-auto relative z-10 pt-4 sm:pt-8 px-2 sm:px-0">
        ...
        <div className="flex items-center gap-3 sm:gap-4 mb-6 sm:mb-8">
          <button
            ...
            className="p-2.5 sm:p-3 bg-gray-100 dark:bg-white/5 hover:bg-gray-200 dark:hover:bg-white/10 border border-gray-200 dark:border-white/10 rounded-full transition-colors group flex-shrink-0"
          >
          ...
          <div>
            <h1 className="text-xl sm:text-2xl font-bold text-gray-900 dark:text-white">
              {isOwnProfile ? 'Your Network' : `${ownerName}'s Network`}
            </h1>
        ...
        <div className="bg-white/60 dark:bg-white/[0.02] backdrop-blur-md border border-gray-200 dark:border-white/10 rounded-2xl sm:rounded-[2rem] p-3 sm:p-6 shadow-xl">
          {isLoading ? (
            ...
            <div className="text-center py-16 sm:py-20">
              {searchQuery ? (
                <>
                  ...
                  <div className="font-bold text-base sm:text-lg text-gray-900 dark:text-white mb-2">No results found</div>
                  <div className="text-sm text-gray-500 dark:text-gray-400">
                    No {activeTab} matching &ldquo;{searchQuery}&rdquo;
                  </div>
                </>
              ) : (
                <>
                  <div className="text-4xl sm:text-5xl mb-4">{activeTab === 'followers' ? '👥' : '🔗'}</div>
                  <div className="font-bold text-base sm:text-lg text-gray-900 dark:text-white mb-2">
            ...
            <div className="divide-y divide-gray-100 dark:divide-white/5">
              {filteredList.map(renderUserCard)}
            </div>
          )}
        </div>
      </div>
