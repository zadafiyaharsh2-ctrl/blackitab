              ...
              <div key={key} className="flex flex-col items-center relative z-10" style={{ minWidth: '44px' }}>
                ...
                <div className="group relative flex flex-col items-center">
                  ...
                  {cfg ? (
                    ...
                    <div className={`w-9 h-9 rounded-full flex items-center justify-center ${cfg.dot} ${cfg.ring} cursor-pointer ${isToday ? 'outline outline-2 outline-offset-2 outline-gray-900 dark:outline-white' : ''}`}>
                      <span className={`text-xs font-bold ${cfg.numColor}`}>{dayNum}</span>
                    </div>
                  ) : (
                    /* No class — small gray dot */
                    <div className={`w-5 h-5 rounded-full bg-gray-100 dark:bg-white/10 border border-gray-200 dark:border-white/15 cursor-pointer flex items-center justify-center mx-auto mt-2 ${isToday ? 'outline outline-2 outline-offset-2 outline-gray-400' : ''}`}>
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8 space-y-6 pt-20">
      ...
      <div className="flex flex-col gap-4">
        ...
        >
          <FaArrowLeft className="text-xs" /> Back to Classes
        </button>
        
        <div className="bg-white dark:bg-white/[0.02] border border-gray-200 dark:border-white/10 rounded-2xl p-6 shadow-sm">
          <div className="flex items-start gap-4">
            <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-blue-500 to-blue-600 flex items-center justify-center text-white shadow-lg shrink-0">
              <AcademicCapIcon className="w-8 h-8" />
            </div>
            <div className="flex-1">
              <div className="flex justify-between items-start">
                <div>
                  <h1 className="text-2xl font-bold text-gray-900 dark:text-white mb-2">{batch.name}</h1>
                  <div className="flex flex-wrap gap-4 text-sm text-gray-600 dark:text-gray-400">
                    ...
                    {batch.section && <span className="px-2 py-0.5 bg-gray-100 dark:bg-white/10 rounded-md font-medium">Section {batch.section}</span>}
                    {batch.classCode && (
                      <span className="px-2 py-0.5 border border-blue-200 dark:border-blue-500/30 text-blue-700 dark:text-blue-400 rounded-md font-mono bg-blue-50 dark:bg-blue-500/10">
                ...
                <button
                  ...
                  className="mt-2 sm:mt-0 px-4 py-2 bg-blue-50 text-blue-600 hover:bg-blue-100 dark:bg-blue-500/10 dark:text-blue-400 dark:hover:bg-blue-500/20 font-medium rounded-lg text-sm transition-colors flex items-center gap-2 shrink-0 border border-blue-200 dark:border-blue-500/20"
                >
                  <FaStar className="text-yellow-500 text-xs" /> Give Feedback
                </button>
              </div>
              
              {/* Quick Attendance Overview */}
              {summary && (
                <div className="mt-6 p-4 bg-gray-50 dark:bg-gray-900/50 rounded-xl border border-gray-100 dark:border-white/5">
                  ...
                  <div className="flex gap-6 mt-3 text-sm text-gray-600 dark:text-gray-400">
                    <span className="flex items-center gap-1.5"><span className="w-2 h-2 rounded-full bg-emerald-500" /> {summary.present} Present</span>
                    <span className="flex items-center gap-1.5"><span className="w-2 h-2 rounded-full bg-red-500" /> {summary.absent} Absent</span>
                    {summary.late > 0 && <span className="flex items-center gap-1.5"><span className="w-2 h-2 rounded-full bg-amber-500" /> {summary.late} Late</span>}
                  </div>
                </div>
              )}
            </div>
