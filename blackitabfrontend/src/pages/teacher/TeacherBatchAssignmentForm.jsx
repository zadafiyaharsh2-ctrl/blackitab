...
const TeacherBatchAssignmentForm = () => {
  ...
  const fetchAssignmentDetails = async () => {
    try {
      const res = await axios.get(`${API_URL}/api/teacher/assignments?batchId=${batchId}`, {
        headers: { Authorization: `Bearer ${localStorage.getItem('token')}` }
      });
  ...
  const handleSubmit = async (e) => {
    ...
    const cleanedData = {
      ...
      batchId: batchId, // Ensure assignment is tied to batch
      status: 'published' // Make it visible to students
    };
    ...
    try {
      ...
      if (isEditing) {
        await axios.put(`${API_URL}/api/teacher/assignment/${assignmentId}`, cleanedData, { headers });
        toast.success('Assignment updated successfully');
      } else {
        await axios.post(`${API_URL}/api/teacher/assignment`, cleanedData, { headers });
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            ...
            <input
              ...
              placeholder="e.g. History Essays"
              className="w-full border border-gray-200 dark:border-white/10 rounded-lg px-4 py-2.5 bg-white dark:bg-white/5 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-orange-500/50"
              required
            />
          </div>
          ...
          <div>
            ...
            <textarea
              ...
              rows="2"
              className="w-full border border-gray-200 dark:border-white/10 rounded-lg px-4 py-2.5 bg-white dark:bg-white/5 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-orange-500/50 resize-none"
            />
          </div>
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5 flex items-center gap-2">
              <FaFileAlt className="text-gray-400" /> Instructions / Questions
            </label>
            <textarea
              name="content"
              value={formData.content}
              onChange={handleChange}
              placeholder="Write detailed instructions or assignment questions here..."
              rows="4"
              className="w-full border border-gray-200 dark:border-white/10 rounded-lg px-4 py-2.5 bg-white dark:bg-white/5 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-orange-500/50"
            />
          </div>
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 flex items-center gap-2">
                <FaLink className="text-blue-500" /> Reference Links
              </label>
              <button
                type="button"
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 flex items-center gap-2">
                <FaFileAlt className="text-emerald-500" /> Reference Document URLs
              </label>
              <button
                type="button"
              ...
              {formData.files.map((file, idx) => (
                <div key={idx} className="flex items-center gap-2">
                  <input
                    ...
                    className="flex-1 border border-gray-200 dark:border-white/10 rounded-lg px-4 py-2 bg-white dark:bg-white/5 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-emerald-500/50 text-sm"
                  />
                  <button
                    type="button"
          ...
          <div className="pt-6 flex justify-end">
            ...
            <button
              type="submit"
              disabled={saving}
              className="flex items-center gap-2 px-6 py-2.5 bg-orange-600 hover:bg-orange-700 text-white rounded-lg font-semibold text-sm transition-colors disabled:opacity-70 disabled:cursor-not-allowed shadow-sm"
            >
...
export default TeacherBatchAssignmentForm;
