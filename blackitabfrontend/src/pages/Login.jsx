      ...
      <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}
        ...
        <div className="text-center mb-8">
          <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ type: "spring", stiffness: 200, delay: 0.2 }}
            className="w-16 h-16 mx-auto mb-4 rounded-2xl bg-gradient-to-br from-blue-500/20 to-indigo-500/20 border border-white/10 flex items-center justify-center shadow-inner">
          ...
          <p className="text-gray-400 mt-2 text-sm">Sign in to continue to Blackitab</p>
        </div>
          ...
          <div className="space-y-1">
            ...
            <div className="relative group">
              ...
              <input type="email" name="email" value={formData.email} onChange={handleChange} required
                ...
                className="w-full pl-12 pr-4 py-3 bg-white/5 border border-white/10 rounded-xl focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500/50 focus:bg-blue-500/5 text-white placeholder-gray-600 outline-none transition-all shadow-inner" />
            </div>
          </div>
          ...
          <div className="space-y-1">
            ...
            <div className="relative group">
              ...
              <input type={showPassword ? 'text' : 'password'} name="password" value={formData.password} onChange={handleChange} required
                ...
                className="w-full pl-12 pr-12 py-3 bg-white/5 border border-white/10 rounded-xl focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500/50 focus:bg-blue-500/5 text-white placeholder-gray-600 outline-none transition-all shadow-inner" />
                ...
                {DEMO_ACCOUNTS.map((d) => {
                  ...
                  return (
                    <button key={d.role} onClick={() => handleDemoLogin(d)} disabled={loading}
                      className={`flex flex-col items-center gap-1.5 p-3 rounded-xl border ${d.border} bg-white/[0.02] hover:bg-white/[0.06] transition-all text-center group`}>
                      <Icon className="w-5 h-5 text-gray-400 group-hover:text-white transition-colors" />
                      <span className="text-xs font-bold text-gray-300 group-hover:text-white">{d.label}</span>
                    </button>
                  );
                })}
              </div>
