...
const ModalContent = ({ children, title, onClose, maxWidth = "max-w-md" }) => (
  <motion.div 
    ...
    className={`w-full ${maxWidth} bg-gray-900/90 border border-gray-700 rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[85vh]`}
  >
      ...
      {isOpen && (
        <ModalBackdrop onClose={onClose}>
          <ModalContent title="Discover People" onClose={onClose} maxWidth="max-w-lg">
            <div className="p-4 border-b border-gray-800 sticky top-0 bg-gray-900/95 z-10 backdrop-blur">
               <form onSubmit={onSearch} className="relative">
                  <FaSearch className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400" />
                  <input 
                    type="text" 
                    placeholder="Search by name or email..." 
                    className="w-full bg-black/40 border border-gray-700 text-white pl-12 pr-4 py-3 rounded-xl focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-all placeholder-gray-500"
                    value={query}
                   ...
                   >
                      <div className="flex items-center gap-3">
                         ...
                         <div>
                            ...
                            <div className="text-xs text-gray-400">{u.followerCount || 0} followers</div> 
                         </div>
                      </div>
                      
                       {currentUserId !== u._id && (
                         u.isFollowing ? (
                            <button className="px-4 py-1.5 rounded-full border border-gray-600 text-gray-400 text-sm font-medium transition-all cursor-default">
                                Following
                            </button>
                         ) : u.isRequested ? (
                            ...
                            <button 
                                 ...
                                 className="px-4 py-1.5 rounded-full bg-white/10 hover:bg-blue-600 text-sm font-medium transition-all text-white border border-white/10 hover:border-transparent"
                            >
                                Follow
                            </button>
                         )
                                            ...
                                            {!note.isAccepted ? (
                                                <>
                                                    <button 
                                                        ...
                                                        className="flex-1 bg-blue-600 hover:bg-blue-500 text-white text-xs font-bold py-2 px-4 rounded-lg transition-colors flex items-center justify-center gap-2"
                                                    >
                                                        <FaCheck size={12} /> Confirm
                                                    </button>
                                                    <button 
                                                        onClick={() => onReject(note.sender._id)}
                                                        className="flex-1 bg-gray-800 hover:bg-gray-700 text-gray-300 text-xs font-semibold py-2 px-4 rounded-lg transition-colors border border-gray-700 flex items-center justify-center gap-2"
                                                    >
                                                ...
                                                !note.isFollowing ? (
                                                    <button 
                                                        ...
                                                        className="w-full bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-500 hover:to-purple-500 text-white text-xs font-bold py-2 px-4 rounded-lg transition-all shadow-lg flex items-center justify-center gap-2"
                                                    >
                                                         <FaReply size={12} /> Follow Back
                                                    </button>
                                                ) : (
                                                    <div className="w-full bg-gray-800 text-gray-400 text-xs font-bold py-2 px-4 rounded-lg flex items-center justify-center gap-2 cursor-default border border-gray-700">
               ...
               {isLoading ? (
                   ...
                   <div className="divide-y divide-gray-800">
                       {users.map((u) => (
                           ...
                           >
                                <div className="flex items-center gap-3">
                                   <div className="w-10 h-10 rounded-full bg-gradient-to-br from-green-400 to-blue-500 flex items-center justify-center text-white font-bold text-sm shadow-md overflow-hidden">
                                      {u.profileImage ? (
                                          <img src={u.profileImage} alt={u.name} className="w-full h-full object-cover" />
                                      ) : (
                                          u.name.charAt(0).toUpperCase()
                                ...
                                {currentUserId !== u._id && (
                                    ...
                                    {u.isFollowing ? (
                                        <button 
                                            ...
                                            className="px-4 py-1.5 rounded-full border border-gray-600 hover:border-red-500 hover:text-red-500 text-gray-300 text-sm font-medium transition-all"
                                        >
                                            Following
                                        </button>
                                    ) : u.isRequested ? (
                                        ...
                                        <button 
                                            onClick={() => onFollow(u._id)}
                                            className="px-4 py-1.5 rounded-full bg-blue-600 hover:bg-blue-500 text-white text-sm font-medium transition-all shadow-lg"
                                        >
                                            Follow
                                        </button>
                                    )}
