...
const Sidebar = ({ onLogout, isOpen, setIsOpen }) => {
    ...
    const fetchUnread = async () => {
      try {
        ...
        const res = await axios.get(
          `${API_URL}/api/social/notifications/unread-count`,
          {
  ...
  const instituteNavItems = canAccessInstitute
    ? [
        ...
        {
          path: "/institute/theory",
          label: "Theory Checking",
          icon: <FaFileAlt />,
        },
        {
          path: "/institute/questions",
          label: "Question Checker",
          icon: <FaClipboardCheck />,
        },
        {
          path: "/institute/complaints",
          label: "Complaints & Feedback",
          icon: <FaExclamationTriangle />,
        },
        {
          path: "/institute/hierarchy",
          label: "Hierarchy",
          icon: <FaSitemap />,
        },
        {
    ...
    <>
      ...
      >
          ...
          {isOpen && (
            <div className="flex items-center gap-2">
              <Logo
                showText={true}
                className="w-8 h-8"
                textSize="text-2xl font-extrabold tracking-tight text-gray-900 dark:text-white"
              />
            </div>
          )}
          <button
        ...
        <nav className="flex-1 px-3 py-4 overflow-y-auto scrollbar-thin scrollbar-thumb-gray-200 dark:scrollbar-thumb-white/10 hover:scrollbar-thumb-gray-300 dark:hover:scrollbar-thumb-white/20">
            ...
            {navItems.map((item) => {
              ...
              return (
                <li key={item.path}>
                  ...
                  >
                    <div
                      className={`relative flex items-center ${isOpen ? "px-4 py-3" : "px-0 py-3 justify-center"} rounded-xl text-[13px] overflow-hidden transition-all duration-200 ${
                        isActive
                    ...
                    >
                      <span
                        className={`relative z-10 text-[16px] ${isOpen ? "mr-3" : ""} ${isActive ? "text-[#0061FF] dark:text-[#a5c3ff]" : "text-gray-400 dark:text-gray-500 group-hover:text-gray-600 dark:group-hover:text-white"}`}
                      >
                        {item.icon}
                      </span>
                      {isOpen && (
                        <span className="flex-1 whitespace-nowrap z-10 leading-tight">
                          {item.label}
                        </span>
                      )}
                    </div>
          ...
          {canAccessTeacher && !hasInstitute && isOpen && (
            <div className="mx-2 mt-4 p-3.5 rounded-xl border border-blue-100 dark:border-[#0061FF]/20 bg-blue-50/50 dark:bg-[#0061FF]/5">
              ...
              <p className="text-[11px] text-blue-600/80 dark:text-[#a5c3ff]/70 leading-relaxed font-medium">
                Link to your institute to access collaborative tools and
                attendance.
              </p>
            </div>
          )}
          ...
          {hodNavItems.length > 0 && (
            <ul className="mt-6 space-y-1.5">
              ...
              {!isOpen && (
                <li className="border-t border-gray-100 dark:border-white/10 mx-4 my-3" />
              )}
                      ...
                      >
                        ...
                        {isOpen && (
                          <>
                            ...
                            {item.badge > 0 && (
                              <span
                                className={`ml-auto text-[10px] font-extrabold px-1.5 py-0.5 rounded-md ${
                                  isActive
                                    ? "bg-white text-[#0061FF]"
        ...
        <div className="p-4 border-t border-gray-200/50 dark:border-white/10 bg-gray-50 dark:bg-transparent">
          ...
          >
            <span
              className={`text-[16px] ${isProfileActive ? "text-[#0061FF] dark:text-[#a5c3ff]" : "text-gray-400 dark:text-gray-500"}`}
            >
              <FaUser />
            </span>
          ...
          <button
            onClick={() => {
              window.dispatchEvent(new Event("openBugReporter"));
              handleNavClick();
            }}
